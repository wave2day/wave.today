...
/* === PATCH: mild tilt in random mode (non-invasive) === */
(function(){
  const ROT_MAX_DEG = 0.55; // <- change this number if you want more/less tilt

  function applyMildTilt(){
    const grid = document.getElementById('eventsGrid');
    if(!grid || !grid.classList.contains('randomMode')) return;

    const posters = grid.querySelectorAll('.poster');
    posters.forEach(p => {
      // If your existing code already sets --rot, we only clamp it.
      const current = (getComputedStyle(p).getPropertyValue('--rot') || '').trim();

      if(current){
        const num = parseFloat(current);
        if(!Number.isNaN(num)){
          const clamped = Math.max(-ROT_MAX_DEG, Math.min(ROT_MAX_DEG, num));
          p.style.setProperty('--rot', clamped + 'deg');
          return;
        }
      }

      // If --rot is not set, set a mild random tilt.
      const rot = (Math.random() * 2 - 1) * ROT_MAX_DEG;
  ...
  window.addEventListener('DOMContentLoaded', () => {
    setTimeout(applyMildTilt, 50);
    setTimeout(applyMildTilt, 250);
  });

  // Re-run after toggles / re-render (captures your existing buttons without touching them)
  document.addEventListener('click', (e) => {
    const t = e.target;
    if(!t) return;
    // Handles clicks on the rndm/date buttons (and their children)
    if(t.closest && (t.closest('#rndmBtn') || t.closest('#dateBtn') || t.closest('.rndmBtn') || t.closest('.dateBtn'))){
      setTimeout(applyMildTilt, 50);
      setTimeout(applyMildTilt, 250);
    }
  }, true);

  // Also watch for grid changes (new posters injected)
  const mo = new MutationObserver(() => applyMildTilt());
  window.addEventListener('DOMContentLoaded', () => {
    const grid = document.getElementById('eventsGrid');

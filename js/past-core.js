...
(function(){
  ...
  // SOFT RANDOM (not overdriven)
  function randomizeVars(){
    Array.from(grid.querySelectorAll('.poster')).forEach((el) => {
      const tx  = (Math.random()*2 - 1) * 8;
      const ty  = (Math.random()*2 - 1) * 12;
  ...
  function openHero(posterEl){
    ...
    requestAnimationFrame(()=>{
      ...
      const w = Math.min(vw-40, rect.width*1.4);
      const h = Math.min(vh-80, rect.height*1.4);
  ...
  async function init(){
    ...
    items = raw.sort((a,b)=>parseDate(b.date)-parseDate(a.date));

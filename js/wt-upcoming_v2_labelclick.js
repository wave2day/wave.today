...
/* GLOBAL_LABEL_CLICK_GUARD */
(() => {
  const isLabel = (t) => !!(t && t.closest && t.closest('.wtLabelWrap, .coreSquare, .wtUpcoming'));
  // pointerdown capture stops link press on mobile
  ...
  document.addEventListener('click', (e) => {
    if(isLabel(e.target)){
      e.preventDefault();
      e.stopPropagation();
      // future hook (calendar)
      const item = e.target.closest('.wtUpcomingItem');
      if(item){
        const dateEl = item.querySelector('.wtLabelDay');
        const monEl = item.querySelector('.wtLabelMonth');
        const payload = {
          day: dateEl?.textContent || null,
          month: monEl?.textContent || null
        };
        if(typeof window.wtOnDateLabelClick === 'function'){
          window.wtOnDateLabelClick(payload, item);
        }
      }
    }
  }, true);
